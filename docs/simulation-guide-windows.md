...
from a bare Windows machine through to a live, highly-available
...
invoice. Today it runs on a single machine, on-premises, with:
...
# Simulation Guide: Windows to AWS, via the Console

Everything below is one continuous walkthrough: a bare Windows
machine, with nothing on it, through to a live, verified, migrated
deployment on AWS, and a full teardown. This same machine plays both
roles, the on-premises "legacy" box and the workstation driving AWS,
unlike the Rocky Linux version of this guide
([`simulation-guide-rocky-linux-10.md`](simulation-guide-rocky-linux-10.md))
where those are usually two separate machines. All resource names use
the `smb-migration-demo-*` prefix; tag everything you create with
...
Starting from a completely bare Windows machine: no git, no code,
nothing installed.

**What you need:** Windows 10 (2004+) or Windows 11, admin rights to
install software, a GitHub account with access to this private repo,
and internet access from this machine.

Install git and the GitHub CLI, using
[winget](https://learn.microsoft.com/windows/package-manager/winget/),
built into Windows 10 2004+ and Windows 11:
...
Open a new terminal afterward so `PATH` picks up both.
...
This is the "before" state: a real box, stateful by design, running
the invoice/customer tracker exactly the way a small business actually
would, just on Windows instead of Linux this time. Same deliberately
unmanaged setup either way: no reverse proxy, no production WSGI
server, SQLite/local sessions/local uploads, one machine, no
supervision beyond a service manager restarting it if it crashes.

Install Python (3.12, well above Flask's minimum of 3.8), if it is not
...
database is empty. Calling `.venv\Scripts\pip.exe`/`python.exe`
directly by path, rather than activating the virtual environment
first, sidesteps PowerShell's script execution policy blocking
`Activate.ps1` on a machine where running scripts has not been
enabled, the same reason this repo's own EC2 deployment scripts call
their venv's binaries directly instead of activating.

**Run it as a Windows service**, so it survives closing the terminal
and restarts itself if it crashes. Windows has no direct equivalent of
systemd for wrapping an arbitrary script as a supervised background
process; [NSSM](https://nssm.cc/) is the standard tool for this, small
and free:

```powershell
winget install -e --id NSSM.NSSM --source winget --accept-package-agreements --accept-source-agreements
```

Open a new terminal afterward, then from inside `legacy-app`:

```powershell
$AppDir = (Get-Location).Path
nssm install legacy-app "$AppDir\.venv\Scripts\python.exe" "$AppDir\app.py"
nssm set legacy-app AppDirectory $AppDir
nssm set legacy-app AppEnvironmentExtra "FLASK_DEBUG=0"
nssm set legacy-app AppExit Default Restart
nssm set legacy-app AppStdout "$AppDir\service-stdout.log"
nssm set legacy-app AppStderr "$AppDir\service-stderr.log"
nssm start legacy-app
```

Setting `FLASK_DEBUG=0` is deliberate: the Flask development server's
debugger is a remote-code-execution risk on anything actually
reachable over a network. `AppExit Default Restart` is NSSM's
equivalent of systemd's `Restart=on-failure`. The two log file
settings matter more here than they would on Linux: there is no
`journalctl` to fall back on, so without them a crash leaves no trace
at all.

**Open the firewall.** Windows Firewall blocks inbound connections to
a listening port by default:

```powershell
New-NetFirewallRule -DisplayName "legacy-app" -Direction Inbound -Protocol TCP -LocalPort 5000 -Action Allow
```

Or scoped to your own network's subnet instead, if you would rather
not expose it more broadly:

```powershell
New-NetFirewallRule -DisplayName "legacy-app" -Direction Inbound -Protocol TCP -LocalPort 5000 -Action Allow -RemoteAddress 192.168.1.0/24
```

**Verify:**

```powershell
Get-Service legacy-app
Invoke-WebRequest -Uri http://localhost:5000/login -UseBasicParsing | Select-Object StatusCode
```

Then from another machine on the network: `http://<this-machine-ip>:5000`
(`ipconfig` shows the IP), log in with `admin` / `changeme123`. Create
a customer, an invoice, and attach a file, so there is something real
to migrate later.
...
instances, by design), and driving the actual data migration. No
second machine needed for this part onward, this same Windows machine
already plays both roles.

```powershell
winget install -e --id Amazon.AWSCLI --source winget --accept-package-agreements --accept-source-agreements
```

Open a new terminal afterward, then confirm:
...
manager, or `python -c "import secrets; print(secrets.token_urlsafe(24))"`),
...
Windows 10 (1803+) and Windows 11 ship `tar.exe` natively, no separate
install needed.
...
editing. This file runs on the Amazon Linux app instance itself once
pasted into the launch template, not on this Windows machine, any text
editor here works fine to change the one line.
...
guessing, from this same machine:
...
Write-Output "Target is healthy"
...
## Part 11: Migrate the Legacy Data

RDS is not publicly accessible by design, so nothing outside the VPC
can write to it directly, not even this machine. Every step that
touches RDS runs on the app instance itself, driven remotely via SSM
(no SSH). This machine's job is relaying the data through S3, since
S3, unlike RDS, is reachable from anywhere.

**1. Stage the data.** Since this same Windows machine ran
`legacy-app` in Part 2, the database and any uploaded file are already
sitting right here, nothing to copy over a network first:

```powershell
New-Item -ItemType Directory -Force migration-data\uploads | Out-Null
Copy-Item legacy-app\instance\app.db migration-data\
Copy-Item legacy-app\uploads\* migration-data\uploads\ -ErrorAction SilentlyContinue
```

(If you installed `legacy-app` on a separate machine instead, Windows
10/11 ship an OpenSSH client, so `scp` works the same way it would on
Linux, `scp your-user@<other-ip>:legacy-app/instance/app.db .`, then
place it under `migration-data\` the same way.)
...
environment variables the systemd unit already set. The app instance
itself is Amazon Linux regardless of what OS this command runs from:
...
Edit** -> Desired capacity: `2` -> Update, or from PowerShell:
...
Needs `boto3` and `requests` installed wherever it runs from:
`modern-app\.venv\Scripts\pip.exe install boto3 requests` covers both
if you do not already have a suitable environment (`modern-app`'s own
venv already has `boto3`, just not `requests`).
...
This Windows machine and the `legacy-app` service running on it are
not AWS-billed, leave it running or stop it as you like
(`Stop-Service legacy-app`), it is not part of this teardown.

## Troubleshooting

**On this Windows machine:** `Invoke-WebRequest` works against
`localhost` but the page will not load from another machine, almost
always the firewall. Confirm the rule exists and applies:
`Get-NetFirewallRule -DisplayName legacy-app | Select-Object Enabled,Profile,Direction,Action`.
A service that will not start shows as `Stopped` in `Get-Service
legacy-app`; check `service-stderr.log` inside `legacy-app\` (from the
`AppStderr` setting in Part 2) for the actual Python traceback, since
there is no `journalctl` equivalent otherwise. A common cause is a
path typo in the `nssm install`/`nssm set` commands, those paths must
match wherever you actually cloned the repo.

**Target group stays unhealthy well past the usual couple of
minutes.** Check the app's own logs via SSM (the app instance itself
is Amazon Linux regardless of what OS this command runs from):
...
- This Windows machine running `legacy-app`: $0 AWS cost, whatever
  this machine already costs to run
